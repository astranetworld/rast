...
| 7 | done: `max_total_wal_size` 256 MiB (`N42_ROCKSDB_MAX_WAL_MB`). Fleet (loop156, full rounds): WAL 1-4 files and at most 0.20-0.23 GiB a node against 28-33 files and 2.25-2.51 GiB on the loop155 build; a node's RocksDB 1.75-1.86 GiB against 3.81-3.96, on 18% more transactions | `docs/FLEET7_PATH_AUDIT.md` section 8 |
...
| 6c | in progress. `verify` holds (loop163 V14: 2,097,152 reads checked, 0 mismatches, 0 declines); `on` holds (loop163-165: 7 of 7 views, 0 invalidations, 0 declines logged). `N42_HASHED_TABLES=off` (96b52c19a) ran four fleet legs with 0 unanswered reads and 0 gas-used mismatches and added 6.5% a round against `on` with the tables written (loop165 means 21.42 -> 22.82M; windows 2-3 +8% / +14%, window 1 unchanged). Next: unwinds and restarts without declines (the change list in "Stage 6c: what turning the hashed tables off takes"), then the setting on by default | `docs/FLEET7_PATH_AUDIT.md` "Stage 6c on the fleet" |
