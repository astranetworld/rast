...
- 阶段 2 的三个杠杆已量过（round 42）：pacing 250 无效（门不是瓶颈）；大块 245k/块 win1 +3–7% 但第 2、3 窗口周期涨到 0.73–1.07 s，不取；body 传输 30 ms，不是杠杆。
  满块 0.41 s 的构成：leader 构建 292–313 ms（串行执行 202–244）+ 自身导入 ~50；follower 导入 220 ms。**下一步是 3A：出块者分组并行执行**。
...
- **round 43 的洪流缺陷**：ingest 路径按连接内局部序号派生收款人，"满块 163k 笔"实际只落到 ~1.3 万收款人；此前所有数字（含 396k）都在这种形状上量得。
  修正（ab3c79240）后满块触及 ~14.7 万账户，round-41 配置读 163–168k TPS、周期 0.97–1.0 s；并行构建不改周期（follower 导入 0.74–0.76 s 成为长杆）。
- **follower 嫁接（`N42_FOLLOWER_GRAFT=1`，344e598d2）**：follower 的分组 bundle 同样嫁接，导入 736–759 → 622–657 ms，win1 201–206k 对 172–182k（两对书挡 +14–17%），周期 0.77–0.81 s。
...
round 43 之后的认识变了，目标随之校正（细节与数据在英文文档 `docs/NATIVE_FLEET7.md`
"Round 43" 及其后各节、`docs/BLOCK_SHAPE_SURVEY.md`）：

- **起点是假的。** 前 42 轮的洪流让 163k 笔只落到 ~1.3 万个收款人；修正后满块触及 ~14.7 万账户
  （与 Tron 的 17 万、以太坊按地址 8.8 万相比是保守形状）。同一套代码在真实形状下从 396k 变成 163–168k。
- **周期随触及账户线性增长：0.40 s + 2.8 µs × 账户**（loop80 扫描）。据此 1,000k（0.163 s/块）在当前架构下不可达：
  光 14.7 万账户的状态提交就是 0.41 s。1,000k 的前提是状态提交完全并行/分片 + 去掉 7 倍冗余的 ingest 验签，
  不是"再优化几个阶段"能到的。
- **已做到（默认开启或记录配置）**：并行出块（3A）、follower 嫁接、并行状态提交、twig 索引哈希、provider 并行
  hashed state、分叉后队列回收、每轮自动记录块形状、出块节奏 450 ms。真实形状下 win1 239–247k、周期 0.65–0.68 s、
  整轮 17.4–17.7M 笔无停顿。leader 满块构建 365 ms，follower 导入 410–470 ms（长杆）。
- **停顿的真因**是 leader 跑赢法定人数之外的 follower（5/7 即可成 QC，最慢两台无限落后），换届时新 leader 追不上、
  超时级联；内存风暴是放大器。已实现"落后者宽限"（`--straggler-grace-ms`，loop88 验证中）。

**校正后的目标与顺序**

1. 稳定性：落后者宽限 / 节奏对齐；每轮块形状自动入档；分叉回收已就位。
2. 真实形状下 **330–350k**：follower 导入 470 → ~300 ms（发送方分组代替连通分量分区、嫁接分片、senders/convert
   并行、twig 结构写入分片）；leader 已非长杆。
3. 供给：ingest ~400k/s/节点会在 330k 后再次成为上限，届时回到 M3 的 ingest 非签名成本与 7× 冗余。
4. 1,000k 重新立项：状态提交分片（每分片独立根、索引分片）、ingest 去冗余、体分片传输——需要单独的设计文档与预算。
