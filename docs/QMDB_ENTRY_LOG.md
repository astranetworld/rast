...
- Step 1 done (`72032c727`): `Entries::{Heap, File}`; `N42_QMDB_ENTRY_FILE=1`. loop123: the
...
- The mapping is chunked (`230363f8f`): sealed 256 MB chunks mapped once with populated page
  tables; loop124 measures it.
- Step 3a done (delta v2 + slot-only undo, `ef280b1c1`): the retired slots are read by nothing
...
- Steps 3b and 4 done (the file as the persistence): `forest.ckpt` = cursor + active bits, the
  ...
  appends buffered and written in one call, `aff1d91f0`: the per-record write syscalls were
