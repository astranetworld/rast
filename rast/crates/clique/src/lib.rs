...
use crate::util::{extract_signers, recover_creator};
...
use crate::traits::EngineSigner;
use unexpected::{Mismatch, OutOfBounds};

// use types::BlockNumber;
pub type BlockNumber = u64;
// use types::header::{ExtendedHeader, Header};
use etheader::ExtendedHeader;
...
impl Engine<EthereumMachine> for Clique {
	...
	fn on_close_block(&self, _block: &mut Block) -> Result<(), Error> {
		Ok(())
	}

	fn on_seal_block(&self, block: &mut Block) -> Result<(), Error> {
	...
	fn generate_seal(&self, block: &Block, parent: &Header) -> Seal {
